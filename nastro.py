...
def process_image(image_path, output_dir=None, show=False):
    ...
    for c in contours:
        ...
        if M["m00"] != 0:
            ...
            imgfinal = cv2.circle(eroded_image_bgr, (cX, cY), 3, (0, 255, 0), -1)
    ...
    if show:
        show_image(
            eroded_image_bgr,
            title=f"{os.path.basename(image_path)} (centros detectados)",
        )
    ...
    if output_dir:
        ...
        # ⬇️ Guardamos la imagen con los círculos y cuadros, NO el overlay
        cv2.imwrite(out_path, imgfinal)
...
output_folder = "carpeta_salida"  # Carpeta donde se guardan los resultados
